...
#Calculate suitable deadlines using an estimate of the average distance between waypoints (1.04 is an arbitrary number)
av_dist = math.sqrt(((lat_upper_bound_x-lat_lower_bound_x)/size)**2+((lon_upper_bound_y-lon_lower_bound_y)/size)**2)
av_time = ((av_dist/speed)+delivery_time+accel_time)*(1.04**size)
